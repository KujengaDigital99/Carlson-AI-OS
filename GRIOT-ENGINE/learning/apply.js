...
/**
 * Scan all amendment files. For any with Status: approved, patch the guardian prompt.
 * Called by the amendment watcher in index.js when an amendment file changes.
 */
async function applyApproved(changedFile) {
  ...
  for (const filePath of target) {
    ...
    // Only act on explicitly approved files
    ...
    // Extract the proposed addition block
    ...
    // Append addition to the prompt (before the STATUS line at the end)
    ...
    // Mark the amendment file as applied
    const updated = content.replace(/\*\*Status:\*\*\s*approved/, '**Status:** applied');
    fs.writeFileSync(filePath, updated, 'utf8');

    // Update patterns.json
    const patternStore = store.load();
    const matched = patternStore.patterns.find(
      p => p.guardian === guardianName && p.trigger === patternSlug
    );
