...
const THRESHOLD       = 3; // occurrences before a proposal is written
...
/**
 * Check patterns.json for any patterns at or above the threshold.
 * For each one without an existing proposal, call Haiku to draft the amendment
 * and write it to learning/amendments/.
 */
async function proposeIfDue() {
  ...
  for (const pattern of pending) {
    console.log(`[PROPOSER] Writing amendment for pattern: ${pattern.trigger} (${pattern.guardian}, count: ${pattern.count})`);

    // Load the current guardian prompt so the proposal has context
    ...
    const content =
      ...
      `3. Save the file — GRIOT Engine will detect the approval and patch the prompt automatically\n` +
